...
if [ -n "$TARGET_RESOURCES" ] && [ ! -w "$TARGET_RESOURCES" ]; then
    warn "Administrative privileges (sudo) are required to patch the application."
    info "Please enter your macOS password when prompted."
    SUDO_PREFIX="sudo"
fi
...
if command -v node >/dev/null 2>&1; then
    info "Using global Node.js to run the patch..."
...
if [ -n "$EMBEDDED_NODE" ]; then
    info "Global Node.js not found. Using Antigravity's embedded Node.js..."
...
warn "Neither global Node.js nor Antigravity app was found in standard locations."
info "Attempting to download a portable Node.js binary to run the patch..."
...
if [ ! -f "$NODE_DIR/bin/node" ]; then
    info "Downloading Node.js $NODE_VER for $ARCH..."
    if command -v curl >/dev/null 2>&1; then
        curl -L "$NODE_URL" -o "$NODE_DIR/$NODE_TAR"
    elif command -v wget >/dev/null 2>&1; then
        wget "$NODE_URL" -O "$NODE_DIR/$NODE_TAR"
    else
        error "Neither curl nor wget is installed. Cannot download Node.js."
        exit 1
    fi
...
if [ -f "$NODE_DIR/bin/node" ]; then
    success "Successfully downloaded portable Node.js. Running the patch..."
    $SUDO_PREFIX "$NODE_DIR/bin/node" "$SCRIPT_DIR/patch.js"
    EXIT_CODE=$?
    exit $EXIT_CODE
else
    error "Failed to set up portable Node.js."
    exit 1
fi
